...
import { BookOpenText, CalendarDays, Cloud, Download, FileText, Flower2, HardDrive, Heart, Home as HomeIcon, Lightbulb, LogOut, Settings2, ShieldCheck, Sparkles, Upload, X } from "lucide-react";
  ...
  async function syncNotes(activeCode: string, pullFirst: boolean) {
    ...
    try {
      while (syncQueuedRef.current) {
        ...
        const { error } = await supabase.rpc("push_test_notebook", { p_code: activeCode, p_payload: { version: 1, records: merged.records, deleted: merged.deleted } });
        if (error) throw error;
